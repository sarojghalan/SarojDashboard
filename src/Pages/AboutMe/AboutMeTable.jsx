...
export const AboutMeTable = ({reload , setReload}) => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
          ...
          setAboutMeData(about);
        });
      }, []);

      console.log("Master Banner Data : ",aboutMeData)
      ...
      function DeleteAction({ data, reload, setReload }) {
        ...
        const deleteAbout = async () => {
          const deleteRef = doc(firebaseDb, "aboutme", data?.id);
          await deleteDoc(deleteRef)
            .then((res) => {
              enqueueSnackbar("ABout Me has been deleted successfulyy", { variant: "success" });
            })
            .catch((err) => {
          ...
          <>
            ...
            <MyModal open={openDelete} setOpen={setOpenDelete}>
              ...
              <SoftTypography color="dark" fontWeight="normal" textTransform="uppercase" mb={2}>
                Do you want to remove this About?
              </SoftTypography>
              <LoadingButton
                title="confirm"
                loading={loading}
            ...
            if (aboutMeData !== []) {
              ...
              for (let i = 0; i < aboutMeData.length; i++) {
                ...
                temp.push({
                  ...
                  "Description": <Description name={classData?.description.substring(0,50)} />,
                  Action: (
                    <>
                      <>
                        <SoftBox
